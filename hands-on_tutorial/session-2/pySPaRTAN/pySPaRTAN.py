...
class pySPaRTAN:
    ...
    def __init__(self, lamda_list=None, rsL2_list=None,  spectrum_list=None, corrtype='pearson',grid_search_steps=10, n_fold=5):
        
       
        self.rsL2_list= rsL2_list
    ...
    def  _cv_grid_search(self, D, P, Y):
        ...
        D_norm = normalize(D.values, norm="l2", axis=0)
        D_norm = pd.DataFrame(D_norm, index=D.index, columns=D.columns)


        # l2 normalize the data by cell
        P_norm = normalize(P.values, norm="l2", axis=1)
        P_norm = pd.DataFrame(P_norm, index=P.index, columns=P.columns)


        Y_norm = normalize(Y.values, norm="l2", axis=1)
        Y_norm = pd.DataFrame(Y_norm, index=Y.index, columns=Y.columns)
        
        n_steps=self.grid_search_steps

         
        lensps = len(self.spectrum_list)
        lenlamdas = len(self.lamda_list)
        lenrsL2s = len(self.rsL2_list)
  
          
        corr_all = np.zeros((lensps, lenlamdas, lenrsL2s))
        
        # for a in range(0, lenspAs):
        for s in range(lensps) :
            for l in range(lenlamdas):
                for r in range(lenrsL2s):
                    print(
                        "cross validating  spectrum={}, lambda={}, rsL2={}".format(
                            self.spectrum_list[s], self.lamda_list[l] ,self.rsL2_list[r]
                        )
                    )
                    Y_pred_all = Y_norm * 0
                    ...
                    result = next(kf.split(P_norm), None)
                    ...
                    for f in range(self.n_fold):
                        ...
                        P_train, P_vali = P_norm.iloc[result[0], :], P_norm.iloc[result[1], :]
                        Y_train, Y_vali = Y_norm.iloc[result[0], :], Y_norm.iloc[result[1], :]
                        ...
                        self._fit(
                            ...
                            P_train,
                            Y_train,
                            lamda=self.lamda_list[l],
                            rsL2=self.rsL2_list[r],
                            # spectrumA=spectrumAs[a],
                            spectrum=self.spectrum_list[s],
                        )
        ...
        self.lamda_best = self.lamda_list[max_l]
        self.rsL2_best = self.rsL2_list[max_r]
        self.spectrum_best = self.spectrum_list[max_s]

        print(f"Best parameters are: spectrum={self.spectrum_best}, lamda={self.lamda_best}, rsL2={self.rsL2_best} " )
    ...
    def get_projD(self, P=None):
        ...
        projD = pd.DataFrame(projD, index=P.index, columns=self.TF_name)
